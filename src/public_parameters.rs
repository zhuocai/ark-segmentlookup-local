...
pub struct PublicParameters<P: Pairing> {
    ...
    pub(crate) g2_srs: Vec<P::G2Affine>,
    // [Z_W(tau)]_2.
    pub(crate) g2_zw: P::G2Affine,
    // q_{i, 2} for i in 1..n*s.
    // The commitment of quotient polynomials Q_{i, 2} s.t.
    // L^W_i(X) * X = omega^i * L^W_i(X) + Z_W(X) * Q_{i, 2}(X).
    pub(crate) g1_q2_list: Vec<P::G1Affine>,
    // q_{i, 3} for i in 1..n*s.
    pub(crate) g1_q3_list: Vec<P::G1Affine>,
    // [L^W_i(tau)]_1 for i in 1..n*s.
    pub(crate) g1_l_w_list: Vec<P::G1Affine>,
    // [(L^W_i(tau) - L^W_i(0)) / tau]_1 for i in 1..n*s.
    pub(crate) g1_l_w_opening_proofs_at_zero: Vec<P::G1Affine>,
    // [L^V_i(tau)]_1 for i in 1..k*s.
    pub(crate) g1_l_v_list: Vec<P::G1Affine>,
    ...
    pub(crate) g1_srs_caulk: Vec<P::G1Affine>,
    pub(crate) g2_srs_caulk: Vec<P::G2Affine>,
...
impl<P: Pairing> PublicParameters<P> {
    pub fn setup<R: Rng + ?Sized>(
        ...
        let g2_zw = vanishing_poly_g2::<P>(&g2_srs, &domain_w);
        ...
        let g1_l_w_list = lagrange_basis_g1::<P::G1>(&g1_srs, &domain_w)?;
        ...
        let g1_l_w_opening_proofs_at_zero =
            zero_opening_proofs::<P>(&g1_srs, &domain_w, &g1_l_w_list)?;

        // Step 5: Compute [L^V_i(tau)]_1 for i in 1..k*s.
        let g1_l_v_list = lagrange_basis_g1::<P::G1>(&g1_srs, &domain_v)?;
        ...
        let tau_pow_n_sub_w_pow_in_list: Vec<P::ScalarField> = (0..order_w)
            ...
            .collect();
        let g1_q3_list: Vec<P::G1Affine> = (0..order_w)
            .map(|i| {
